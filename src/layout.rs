...
This module implements basic CSS box model in the form of a layout tree.

Rules:
- box elements have dimensions and location: width, height, and xy coordinates
- box elements also can have padding, border, and margin values; defined in CSS delcarations, but live in separate structure?
- block elements are placed vertically
- inline elements placed horizontally
- inline elements must only contain block elements or inline elements; in the case of mixed content, anonymous block renderes will be created to wrap inline elements


The layout tree is similar to style tree in that each element contains children of the same type as itself
...
// Note: All size / dimension units are in px.
